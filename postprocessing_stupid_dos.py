...
def main():
    ...
    stupid_dos = np.zeros([n_d, NE], dtype=np.float)
    ...
    if os.path.isfile('postprocessing/stupid_dos.dat'):
        stupid_dos[:, :] = np.loadtxt('postprocessing/stupid_dos.dat')
    else:
        stupid_dos[:, :] = get_average_distribution('stupid_DOS_0', n_d, n_r, NE)  # the same for LUMO+
        for i_d in range(0, n_d):
            stupid_dos[i_d, :] /= aaa[i_d]
        np.savetxt('postprocessing/stupid_dos.dat', stupid_dos)
